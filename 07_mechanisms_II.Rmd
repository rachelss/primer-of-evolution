...
```{r migfigprep, include=FALSE, warning=FALSE}
migration_rates <- c(0.010,0.025,0.100,0.500)
pm0 = 0.05
pi0 = 0.95
...
## Case Study: Modeling Mutation, Drift, and Selection

This section will teach you how to model evolutionary forces beyond selection, with particular emphasis on mutation and genetic drift. You will use simulations to work through a number of scenarios that look at the effects of evolutionary forces, both in isolation and in conjunction with selection:
...
All models you will run for this chapter's exercise are implemented in the R package `learnPopGen`. 
...
In addition to selection, allele frequencies in natural populations are impacted by mutation, genetic drift, and migration. In this section, we explore the first two of these in more detail.

Mutation alone has a weak impact on allele frequencies. The reason for the weak effect lies in the rarity of specific mutations. Mutations that change allele *A* to *a*---or vice versa---are simply not common enough to move the needle on the allele frequency scale.

Still, mutations do occur, and because most of them are deleterious, selection acts to eliminate them from the population. Once selection gets rid of deleterious alleles, mutation eventually puts the allele back into the gene pool. When the rate of elimination is equal to the rate of mutation, the frequency of an allele is at an equilibrium that we call the mutation-selection balance. The frequency of the deleterious allele *a* (*q*) is dependent on the mutation rate and the fitness of the different genotypes.

We can simulate the mutation-selection balance for different mutation rates and different strengths of selection using the `mutation.selection()` function. 

-   You need to designate a starting allele frequency (`p0`). This value should be between zero and one.
Because the model allows for mutation, even if one allele starts out as either the only present allele or not present at all, mutation can create this allele.

-   You need to designate how many generations you want a simulation to run (`time` argument).

-   You need to define the fitness (`w`) of the different genotypes. 
The `mutation.selection()` function automatically uses the *AA* genotype as the reference (*w*~AA~=1). Hence, you only need to generate a vector with two numbers, designating the fitness of *w*~Aa~ and *w*~aa~: `fitness <- c(1+s1, 1+s2)`.
...
When modeling mutation-selection balance, we are typically interested in the frequency of the deleterious allele (*a*), and you can plot *q* directly by using `show="q"`. You will need to generate multiple plots to compare the outcomes of different models.

This initial simulation has a starting allele frequency of *A* (*p*) of 1, a mutation rate of 0.001, and selection against *aa* (*w*~aa~ = 0.5) with the heterozygotes having the same fitness as AA (*w*~Aa~ = 1).

**Homework problem 1: What happens to the deleterious allele if you hold fitness constant and you vary the mutation rate? Run as many simulations as you need to make out a pattern.**

**Homework problem 2: What happens to the deleterious allele if you hold mutation rate constant and you vary the strength of selection against *aa*? Run as many simulations as you need to make out a pattern.**

**Homework problem 3: What happens to the deleterious allele if the heterozygotes have a higher fitness than AA? Run as many simulations as you need to make out a pattern.**

### Modeling Genetic Drift

In the simulations so far, we have been looking at changes in allele frequencies assuming infinitely large population sizes (hence, we never actually specified the population size in our models). Now we are going to see how population size influences evolutionary outcomes in absence of selection. Population size primarily impacts the importance of chance events (*i.e.*, genetic drift) in evolution. To illustrate this, here is a coin toss analogy. Of course the chance of getting head or tail is 50/50. If you throw a coin just a few times, you may get a huge deviation from the expected frequency of tail; instead of 50%, you may find you get tail 4, 5, or even 6 times in a row. If you toss your coin dozens or hundreds of times though, you will approach the expected frequency of 50% closer and closer (unless you have a funky coin). So, as the number of events increases, the relative impact of individual chance events gets lower and lower.

Let's investigate the evolutionary consequences of genetic drift and how it varies as a function of population size. We can model drift with the `genetic.drift()` function to model the effects of drift as a function of population size. As before, parameters for this function include the starting allele frequency (`p0`; between 0 and 1), and `time` (number of generations you want the simulation to run). In addition, models of genetic drift require two additional inputs:
...
Running the simulations works the same way as the previous functions you used. As an example, here is a 50 generation drift model with an initial allele frequency to *p*~0~=0.5 and a population size of 20. 
...
Based on the example above, model genetic drift for a variety of population sizes and answer the following questions in your **homework**. I recommend starting with a lower number of generations (\<100) and replicates (\~5) because these simulations are a bit more intensive computationally.

4. What consistently happens to allele frequencies in response to drift across all simulations you ran? 
5. What are the primary differences in how allele frequencies change as populations get smaller? 
6. What are the long-term consequences of genetic drift? 
7. As you compare all of your simulations, what are your key conclusions about how drift impacts allele frequencies over time?

*tip: As you increase population size, you may also want to increase the number of generations that are modeled.*
...
To conclude, we're going to look at the interaction between genetic drift and natural selection. We'll use the Devil's Hole pupfish as a case study; as you know, these fish had a low-bound population size of only 38 individuals.

To simulate the combined effects of genetic drift and selection using the `drift.selection()` function, which combines a number of input parameters that you are now already familiar with:
...
start.freq = 0.001 # for the A allele
fitness=c(1.8,1.5,1) # fitness of the three genotypes: AA, Aa, and aa
generations = 50
popsize = 200
n = 5 # number of times the simulation is repeated

r3 <- drift.selection(p0=start.freq,Ne=popsize,w=fitness,ngen=generations,nrep=n)
```

In the population of pupfish fixed for *A*, imagine that a random recessive mutation (*a*) arises (i.e. occurs in one individual in the pupfish population). The mutation provides an 15% fitness benefit to homoyzgous individuals and a 5% benefit to heterozygous individuals. Using the `drift.selection()` function, model how the beneficial mutation spreads through the population. 

**Homework 8: What happens to the beneficial mutation? How do your results change as the population size increases? Run as many simulations as you need to understand the potential outcomes.**

Now assume the same scenario, but the new mutation has a fitness cost of 2% for homozygous individuals.

**Homework 9: What happens to the deleterious mutation? How do your results change as the population size increases? Run as many simulations as you need to understand the potential outcomes.**

*Note: Consulting additional resources to answer the homework questions is  allowed, but failure to disclose those resources is plagiarism. Please list any collaborators you worked with and resources you used or state that you have not used any.*
