...
Running the simulations works the same way as the previous functions you used. As an example, here is a 100-generation drift model with an initial allele frequency to *p*~0~=0.5 and a the population size of 50. I recommend starting with a lower number of generations (\<100) and replicates (\~5) because these simulations are a bit more intensive computationally.
...
Based on the example above, model genetic drift for a variety of population sizes and answer the following questions in your **homework**:
...
In the population of pupfish fixed for *A*, imagine that a random recessive mutation (*a*) arises. The mutation provides an 8% fitness benefit to homoyzgous carriers. Using the `drift.selection()` function, model how that beneficial mutation spreads through the population. 

**Homework 8: What happens to the beneficial mutation? How would your results change as the population size increases?**

Now assume the same scenario, but the new mutation has a fitness cost of 8 %.

**Homework 9: What happens to the deleterious mutation? How would your results change as the population size increases?**
