...
## Practical Skills: Plotting with ggplot

### R Basics

In the [last chapter](what-evolution-is.html#r), you learned how to enter commands in the RStudio console to receive an output from R. This showed you the general principle of how you can prompt R to execute any function you want.

#### Loading Libraries {.unnumbered}

To make use of installed libraries, you also need to load the libraries *every time* you use R (*i.e.*, every time you restart the program). You can do this with the `library()` command. The following code chunk loads the `tidyverse` library with most of the basic functions you will need for plotting and data manipulation:
...
To import data in the form of \*.csv files, you can use R's `read_csv()` function. In the code chunk below, you can import a simple test data set [test_data.csv](data/test_data.csv) provided with this textbook that includes three variables: sex, length, and mass of individuals in a population.
...
#The line of code simply prompts the computer to read the "test_data.csv" file and generate a data.frame called test_data
test_data <- read_csv('https://raw.githubusercontent.com/rachelss/primer-of-evolution/main/data/test_data.csv')
```
...
A key learning objective of this course is that you learn to visualize and interpret data to address different evolutionary hypotheses. In the following sections, I will explain step by step (that is code line by code line) how to make a simple graph with our test data. Let's aim to make a scatter plot showing the relationship between length and mass of individuals in the population. The process is not much different than sketching a graph by hand and layering different parts of the graph on top of each other, just that you use words (code) to make the computer draw. To graph data, we will primarily use the `ggplot()` function that comes with the `ggplot2` library, which is packaged in the `tidyverse`.
...
In the exercise associated with this chapter, you will not be plotting data from individuals but rather aggregate data that is compiled from many individuals and provides a mean and a measurement of variation around a mean for different sampling groups. To show you how we can visualize such data as mean (± variation), we need to first calculate the mean and standard deviation (sd) of length separate for each sex based on the data above.
...
Next we calculate the mean and sd for each group, and put this into a new variable.
...
```{r sm}
summary_test_data <- summarise(grouped_test_data,mean=mean(length),sd=sd(length))

```

Click on the variable in your Environment to view your new summary table.

To visualize means and standard deviations, we can again use the `ggplot()` function with sex on the x axis and the mean value of length on the y axis (note that we are now referring to the `summary_test_data` data frame that we just created). As before, we are using `geom_point()` to draw our data as points. In addition, we are using `geom_errorbar()` to draw the standard deviations around the mean in both directions. You already know all the other code elements from above.

```{r meanplot}
ggplot(summary_test_data, aes(x=sex, y=mean)) +
  geom_point() +
  geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=0.1)  +  #width designates the width of the horizontal bars
  xlab("Sex") +
  ylab("Mean length [mm]")
```

You can't say whether the mean values for male and female length are different from the graph alone.
Obviously, the mean values are not equal, but whether they are significantly different depends on the variation in the samples as well as the number of samples.
We observe that the error bars overlap, but that doesn't mean that the mean values are or are not different.
...
What do you conclude about whether males and females differ?
