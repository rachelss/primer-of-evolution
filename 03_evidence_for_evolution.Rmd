...
To do this, we will use the `group_by` and `summarize` functions to create a new variable where each subset of data is labeled. First, tell `group_by` the name of the data you want to divide into subsets, followed by the columns you want to include.
...
ggplot(summary_test_data, aes(x=sex, y=mean)) +
  geom_point() +
  geom_errorbar(aes(ymin=mean-sd, ymax=mean+se), width=0.1)  +  #width designates the width of the horizontal bars
  xlab("Sex") +
  ylab("Mean length [mm]")
```
