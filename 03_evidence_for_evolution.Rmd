...
Note that this week's case study also provides a general introduction to RStudio. The practical skills required to complete the exercise are also explained in the section below.
...
## Plotting as the basis for understanding relationships in data

During this course, you will learn about evolution *and* practice being a scientist by visualizing and interpreting data on a weekly basis. To do so, we will use an open-source software called R. This section provides instructions for accessing R using URI's RStudio web server.

### What are R and RStudio?

R is a programming language for statistical analysis and data visualization, and over the past decades, it has become one of the most critical and universally used tools in the life sciences. RStudio is a convenient interface that allows you to easily write, organize, and execute your R code; it's the program we will use throughout the semester for weekly assignments. 

### Accessing RStudio

* Go to [https://rstudio.uri.edu/](https://rstudio.uri.edu/)
* Log in using your URI email address minus the `@uri.edu`
...
Learning a programming language---like learning any language---can be intimidating. However, it is a basic skill for any scientist. 
In this book, you will work through R exercises that help you digest the basic concepts discussed in each chapter. We will show you how to use R to work through the exercises. Assignments in class will provide you with the code to produce a desired outcome, which you will then reinforce with exercises outside of class. We encourage you to explore beyond what the exercises dictate. A willingness to experiment will make you a better programmer and scientist. 

#### The Console and Basic R Prompts {.unnumbered}

![](images/rstudio_interface.png)

Let's begin with the fundamental building blocks of R and RStudio. As you open RStudio, the program window will look something like the figure above. The panel in the upper right contains your work space (Global Environment) as well as a history of the commands that you've previously entered. Any plots that you generate will show up in the panel in the lower right corner and you can also use that section to browse your folders for specific files. The panel on the left is where the action happens. It's called the Console. Every time you launch RStudio, it will have the same text at the top of the console telling you the version of R that you're running. Below that information is the command prompt (symbolized by `>`), where any commands you run will produce output.

Let’s start by getting organized. Because RStudio provides a mechanism for setting up projects we will use this to stay organized. 

* In the RStudio File menu select New Project - New Directory - New Project.
* Give the project a name (e.g. BasicGraphs). For this and all future work ensure that names never include spaces.
* Create Project. By creating a new project you have created a folder for your work. When using projects whenever you come back to this project you will return to the same setup. This allows you to switch projects with different folders and open files.
* From the File menu select New File - R script to show a fourth pane in the upper left corner.
* Save your script (it will be saved automatically in your project folder). Now, when you type in this script all your code will stay here. From now on type your code in your script and click the `Run` button to run it.

#### Working with Variables {.unnumbered}

When you load data into R it stores it as a variable. Variables are symbols that stand for values. You can designate a variable using `<-`. For example, we can assign the value 5 to x:

```{r basic3}
x <- 5
```

Notice when you run that code, you created a new variable x with the value 5 that shows up in your Environment on the top-right panel. Once you have a variable defined, you can use it in calculations.
For example, this code calculates x squared.

```{r basic4}
x^2
```

You can also evaluate information about the variable. The logic statement below asks whether x is equal to 5 (note the double equals). This statement should return True.
...
Variables are not only able to designate numbers, they can also designate lists of numbers (called vectors). You can create a vector (`y` in this case) using the `c()` function, and you can then call on `y` to display the vector in the console:
...
Note that if `x` was a list of numbers (referred to as a vector), the function is applied to every number in the vector.
...
Note that this code again created a new object called `sqrt_y` in your work space, and you can call on it to display it in the console. We can now also use the new values as input for another function. For example, we can calculate the average of these values using the `mean()` function:
...
In the [last chapter](what-evolution-is.html#r), you learned how to enter commands in the RStudio console to receive an output from R. This showed you the general principle of how you can prompt R to execute a function to analyze data.

#### Loading Libraries {.unnumbered}

Much like you can install apps on your phone to provide additional functionality, 
you can use libraries in R to add additional tools.
To make use of installed libraries, you need to load the libraries *every time* you use R (*i.e.*, every time you restart the program). You can do this with the `library()` command. The following code chunk loads the `tidyverse` library with most of the basic functions you will need for plotting and data manipulation:
...
To import data in the form of \*.csv files, you can use the `read_csv()` function. Using the code chunk below, you can import a simple test data set [test_data.csv](data/test_data.csv) provided with this textbook that includes three variables: sex, length, and mass of individuals in a population.
We will use this dataset to examine how length and mass are related in some animals,
as well as how size is related to sex.

```{r readcsv, echo = FALSE}
#The line of code simply prompts the computer to read the "test_data.csv" file and generate a data.frame called test_data
test_data <- read_csv('data/test_data.csv')
```

```{r readcsv2, eval = FALSE}
#The line of code simply prompts the computer to read the "test_data.csv" file and generate a data.frame called test_data
test_data <- read_csv('https://tinyurl.com/primerevotestdata')
```

If this worked correctly, you should now see a new data frame called `test_data` in your work space (Global Environment; top right panel). You can double click it to view it or execute `View(test_data)` in the console as described on [Chapter 1](what-evolution-is.html#data-frame). There should be three columns: sex, length, and mass.

### Graphing Data

A key learning objective of this course is that you learn to visualize and interpret data to address different evolutionary hypotheses. 
To understand if there is a relationship between length and mass of individuals in the population
we can use a scatter plot. The process of constructing a graph in R is not much different than sketching a graph by hand and layering different parts of the graph on top of each other, except that you use words (code) to make the computer draw the graph. To graph data, we will primarily use the `ggplot()` function that comes with the `ggplot2` library, which is packaged in the `tidyverse`.
...
In this case we are using the graph to examine if mass is related to length in our samples.
...
The second step is to draw the actual data into the established coordinate system. To do so, you need to tell the program what kind of graph you want to draw. Different graph types in ggplot are referred to as geoms (geometries), and a scatter plot is designated as `geom_point()`. You can add that to your existing code describing the coordinate system with a plus sign.
...
While there appears to be a relationship between length and mass, we cannot say this for sure without using statistics.

#### Adding Additional Complexity {.unnumbered}

If you look at the data frame, you will see that we do not only have information about the length and mass of individuals in the population, but also their sex. 
Based on experience we may hypothesize that we should account for potential sex differences in the relationship between length and mass. To do so, we can color-code individual points based on the sex of the individual by adding another term of the aesthetics of the `ggplot()` function (`color=sex`):

```{r groups, message=FALSE}
ggplot(test_data, aes(x=length, y=mass, color=sex)) +
  geom_point() +
  geom_smooth(method="lm", se=FALSE) +
  xlab("Body length in cm") +
  ylab("Body mass in kg")
...
As you can see, this not only changes the color of individual points, but it also draws separate regression lines for males and females. If you want to change the legend title, you can do this again with the `labs()` function: `labs(x="Body length in cm", y="Body mass in kg", color="Sex")` .

#### Changing to a Colorblind-Friendly Palette {.unnumbered}

ggplot uses a default color palette when you add color to a graph. 
Because some people have impaired color perception it's important to present your data
in a way that is accessible to all.
A simple way of changing the color palette of your graph is to add viridis coloration to your plot. 

To change the color scheme of the graph generated with the `ggplot()`, you can simply use the `+ scale_colour_viridis_d()` function to designate the desired palette:
...
Note the `d` at the end of the function stands for discrete as you have two discrete categories of data.

#### Generating and Visualizing Aggregate Data {.unnumbered}

In the exercise associated with this chapter, you will not be plotting data from individuals but rather aggregate data that is compiled from many individuals.
You can then compare the means for different sampling groups
using the variation among samples for each group to determine whether they are different. 
To show you how we can visualize means and variation, we will calculate these values for each group (sex) in our data.

To do this, we will use the `group_by` and `summarize` functions to create a new variable where each subset of data is labeled. First, tell `group_by` the name of the data you want to divide into subsets, followed by the columns you want to include. R will make groups based on the different categories in these columns.


```{r gb}
grouped_test_data <- group_by(test_data, sex)
```

Next we calculate the mean, standard deviation (sd), and standard error (se) for each group, and put these into new variables.
The name of the column containing the mean (calculated using the function `mean`) is "mean".
The name of the column containing the sd (calculated using the function `sd`) is "sd".
The name of the column containing the se (calculated by dividing sd by the square root of the sample size (n)) is "se" (note that n() is a function because it's calculating the sample size).

The standard deviation is a representation of the amount of variation in the data.
The mean plus/minus 2sd contains approximately 95% of the data for the sample.
The standard error is an estimate of the accuracy of the estimate of the mean. 
The function to calculate the standard error includes both variation (sd) and the size of the sample.
That means that as you increase the sample size the estimate of the mean gets more precise,
even though the amount of variation in your sample (sd) may stay the same.
The mean plus/minus 2se is the 95% confidence interval for the estimate of the mean.
That means that we expect the actual estimate of the mean falls within this range.
When you are comparing means from two samples as we do here, it's important to compare the two confidence intervals not the means themselves. 


```{r sm, echo=FALSE}
#for some reason needs dplyr to run correctly in rmd
summary_test_data <- dplyr::summarise(grouped_test_data, mean=mean(length), sd=sd(length), se=sd(length)/sqrt(n()))
```

```{r sm_alt, echo=TRUE, eval=FALSE}
summary_test_data <- summarise(grouped_test_data, mean=mean(length), sd=sd(length), se=sd(length)/sqrt(n()))
```

Click on the variable in your Environment to view your new summary table.

```{r s_table}
summary_test_data
```

To visualize the means and confidence intervals (+/- 2SE), we can again use the `ggplot()` function with sex on the x axis and the mean value of length on the y axis (note that we are now referring to the `summary_test_data` data frame that we just created). As before, we are using `geom_point()` to draw our data as points. In addition, we are using `geom_errorbar()` to draw the standard deviations around the mean in both directions. You already know all the other code elements from above.

```{r meanplot}
ggplot(summary_test_data, aes(x=sex, y=mean)) +
  geom_point() +
  geom_errorbar(aes(ymin=mean-2*se, ymax=mean+2*se), width=0.1)  +  #width designates the width of the horizontal bars
  xlab("Sex") +
  ylab("Mean length [mm]")
```

You can't say whether the mean values for male and female length are different from the graph alone.
To determine whether the mean values are different we need to use a statistical analysis.
When we have two categories of data we can use a t-test with the `t.test` function in R.
The null hypothesis is that the two means are equal, and the alternative is that they are not.

The t.test function takes two "arguments" - one vector of each category of data.
To get our first vector we need to `filter` our original test dataset for females (i.e. cases where we observe the value 'female' in the `sex` column).
Because we only have the mean values in the summary dataset we cannot use this dataset to determine if the means are different as we need information about sample size and variation across samples.

```{r filter}
female_test_data <- filter(test_data, sex == 'female')
```

Now you should be able to repeat this to get `male_test_data`.

```{r filter2, echo=FALSE}
male_test_data <- filter(test_data, sex == 'male')
```

Now we can compare the `length` columns of these two datasets.

```{r t}
t.test(female_test_data$length, male_test_data$length)
```

The simplest way to interpret the output of the t test is to focus on the p-value.
If the p-value is less than a typical alpha value of 0.05 we will reject the null hypothesis
that the mean values are the same. 
We define our alpha value as the value at which we are willing to reject the null hypothesis if it is true.

**Make a statement about your null hypothesis and your conclusion about whether males and females differ in length.**

## Case study: Darwin's Finches

One of the most iconic study systems in evolutionary biology are Darwin's finches on the Galapagos Islands. Rosemary and Peter Grant spent much of their lives devoted to the study of these birds, examining how their traits change in response to major ecological perturbations. To do so, they collected a massive, long-term data set on different traits of the medium ground finch (*Geospiza fortis*) population on Daphne Major Island. For this exercise, we will take a look at their beak size data from 1972-1994.

![](https://raw.githubusercontent.com/rachelss/primer-of-evolution/main/finch.jpg)

### Import data

The beak size data can be found in file called "finches.csv" located at 'https://raw.githubusercontent.com/rachelss/primer-of-evolution/main/data/exercises/finches.csv'. The file includes three variables: year, the average relative beak size (rel.beak.size), and the standard error (st.err) that describes the variability of beak size in any given year. Given the work you have done previously, you should be able to read this file into a variable in your Environment.

```{r, echo = FALSE, warning=FALSE, message=FALSE}
finch <- read_csv("data/exercises/finches.csv")
```


### Plotting the Data

Given the work you have done previously, you should be able to 
make a scatter (point) plot of your data relating the
year and the average relative beak size.
...
### Adding Additional Graphical Elements

There are two graphical elements that we can add to facilitate the interpretation of the data:

1.  Because this is a time series, it makes sense to connect the dots representing the means from year to year. You can do this by simply adding another geom: `geom_line()`.
2.  We want to know how much the average beak size changes relative to the variability in the population. If variability is high, year to year variation in may be negligible. But if variability is low, changes across year may actually be substantial. Add `geom_errorbar()` as in your prior work to show the variability.
