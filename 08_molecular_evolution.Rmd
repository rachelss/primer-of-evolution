...
***Note that if you are using the URI RStudio Server you should check that you are using R 4.1.2***
...
#Read in sequences
hemoSeq <- Biostrings::readAAStringSet("~/../shared/272/msa_examples/HemoglobinAA.fasta")
```

```{r message=FALSE, warning=FALSE, echo = FALSE}
#Read in sequences
hemoSeq <- Biostrings::readAAStringSet("data/HemoglobinAA.fasta")
```
...
```{r d_tree, out.width='100%', fig.cap="Neighbor-joining tree of different vertebrates based on hemoglobin ⍺1 sequences."}
library(ape)
...
```{r numtree, echo=FALSE, warning = FALSE, message = FALSE, fig.cap="Number of possible tree configurations as a function of the number of taxa in a data set, as calculated by Equation (7.1)."}
n <- 3:60
...
test.tree <- phytools::read.newick("https://raw.githubusercontent.com/rachelss/primer-of-evolution/main/data/7_test_tree.nwk")
```
...
The [metadata for the example](7_metadata.csv) tree used here can be imported using the `read.csv()` function that you are already familiar with. As you can see below, it provides information about the geographic distribution and dietary habits of each species in the phylogeny.

```{r}
meta.data <- read.csv("https://raw.githubusercontent.com/rachelss/primer-of-evolution/main/data/7_metadata.csv")
head(meta.data)
```
...
```{r fig.height=6, fig.width=8}
tree  %<+% meta.data + geom_tiplab(aes(label=diet), offset=0) + xlim(NA,500)
```
...
```{r fig.height=6, fig.width=8}
tree  %<+% meta.data + geom_tiplab() + geom_tiplab(aes(label=diet), offset=240) + xlim(NA,750)
```

Finally, we can also color code the tips of the phylogeny based on an input variable using `geom_tippoint()`:

```{r fig.height=6, fig.width=8}
tree  %<+% meta.data + geom_tiplab(offset=20) + geom_tiplab(aes(label=diet), offset=370) + xlim(NA,900) + geom_tippoint(aes(color=origin), na.rm = TRUE)
```
